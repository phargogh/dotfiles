# TODO: install fzf without user-interaction
set -e

mkdir -p workspace/phargogh

# wget is available on ubuntu by default (as far as I can tell)
sudo apt update
sudo apt install -y git vim curl vim-youcompleteme zsh zsh-syntax-highlighting
